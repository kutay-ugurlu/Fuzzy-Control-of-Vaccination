...
class Vaccination(object):
    ...
    def __init__(self):
        ...
        # percentage of the vaccinated people
        self.vaccinated_percentage_curve_ = [0.]
    ...
    vaccination_control: float, vaccination rate to be added to the current vaccination rate
        ...
        vaccination_rate = max(
            0., min(.6, curr_vaccination_rate + vaccination_control))
        ...
        vaccination_percentage = \
            min(1., self.vaccinated_percentage_curve_[
                -1] + effective_vaccination_rate * .1)
    ...
    def viewVaccination(self, point_ss, vaccination_cost, save_dir='', filename='vaccination', show_plot=True):
        ...
        color_list = ['#ff0000', '#32CD32', '#0000ff',
                      '#d2691e', '#ff00ff', '#000000', '#373788']
        style_list = ['-', '--']
        ...
        ax = axes[0]
        ax.set_title('vaccinated population percentage over days',
                     loc='left', fontsize=font_size)
        ax.plot(day_x[:point_ss+1],
                self.vaccinated_percentage_curve_[:point_ss+1], **plot_curve_args[0])
        ax.plot(day_x[point_ss:], self.vaccinated_percentage_curve_[
                point_ss:], **plot_curve_args[1])
        ax.plot([day_x[point_ss]] * 2, [0,
                                        self.vaccinated_percentage_curve_[point_ss]], **plot_vert_args[2])
        ...
        # vaccination rate
        ax = axes[1]
        ax.set_title('vaccination rate over days',
                     loc='left', fontsize=font_size)
        ...
        ax = axes[2]
        ax.set_title('vaccination rate control over days',
                     loc='left', fontsize=font_size)
        ...
        ax.set_xlabel(xlabel='day', fontsize=font_size)
        ax.set_ylabel(ylabel='vaccination rate control (%/day)',
                      fontsize=font_size)
        ax.set_xticks(x_ticks)
        ax.xaxis.set_minor_locator(ticker.FixedLocator([day_x[point_ss]]))
        ax.xaxis.set_minor_formatter(ticker.ScalarFormatter())
